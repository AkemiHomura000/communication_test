                      ...
                      std::unique_ptr<tf2_ros::Buffer> &tf_buffer, rclcpp::Logger logger_)
{
  ...
  if (!transform_acquired)
  {
    ...
    try
    {
      livox_to_base_link_transform = tf_buffer->lookupTransform("livox_frame", "base_link", odomAftMapped.header.stamp);
...
int main(int argc, char **argv)
{
  ...
  auto srv_reset = nh->create_service<std_srvs::srv::Trigger>(
      "/reset_map_and_odom",
  ...
  while (rclcpp::ok())
  {
    ...
    if (!init_map)
    {
      ...
      else
      {
        ...
        RCLCPP_INFO(logger, "initial map size: %d", init_feats_world->size());
    ...
    if (use_imu_as_input)
    {
      RCLCPP_WARN(logger, "Using IMU as input state, not supported yet!");
      break;
    }
    ...
    if (time_seq.size() > 0)
    {
      ...
      for (k = 0; k < time_seq.size(); k++)
      {
        ...
        if (publish_odometry_without_downsample)
        {
          ...
          if (duration_since_last_publish >= 1) // 1000hz
          {
          ...
          if (runtime_pos_log)
          {
            ...
            fout_out << setw(20) << Measures.lidar_beg_time - first_lidar_time << " " << euler_cur.transpose()
                     << " " << kf_output.x_.pos.transpose() << " " << kf_output.x_.vel.transpose() << " "
    ...
    //           fout_out << setw(20) << Measures.lidar_beg_time - first_lidar_time << " " << euler_cur.transpose()
    //                    << " " << kf_input.x_.pos.transpose() << " " << kf_input.x_.vel.transpose() << " "
    //                    << kf_input.x_.bg.transpose() << " " << kf_input.x_.ba.transpose() << " "
    //                    << kf_input.x_.gravity.transpose() << " " << feats_undistort->points.size() << endl;
    //         }
    //       }
    //       for (int j = 0; j < time_seq[k]; j++)
    //       {
  ...
  fout_imu_pbp.close();
  rclcpp::shutdown();
  RCLCPP_INFO(logger, "pointlio finished");
  return 0;
}
